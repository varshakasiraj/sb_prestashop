...
include_once(_PS_MODULE_DIR_.'angarslider/AngarSlide.php');

class AngarSlider extends Module
{
    protected $_html = '';
    protected $default_width = 1920;
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        $this->displayName = $this->l('AT - Slider');
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('ANGARSLIDER_WIDTH', $this->default_width, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('ANGARSLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('ANGARSLIDER_PAUSE', $this->default_pause, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('ANGARSLIDER_LOOP', $this->default_loop, false, $shop_group_id, $shop_id);
            }

            /* Sets up Shop Group configuration */
            if (count($shop_groups_list)) {
                foreach ($shop_groups_list as $shop_group_id) {
                    $res = Configuration::updateValue('ANGARSLIDER_WIDTH', $this->default_width, false, $shop_group_id);
                    $res &= Configuration::updateValue('ANGARSLIDER_SPEED', $this->default_speed, false, $shop_group_id);
                    $res &= Configuration::updateValue('ANGARSLIDER_PAUSE', $this->default_pause, false, $shop_group_id);
                    $res &= Configuration::updateValue('ANGARSLIDER_LOOP', $this->default_loop, false, $shop_group_id);
                }
            }

            /* Sets up Global configuration */
            $res = Configuration::updateValue('ANGARSLIDER_WIDTH', $this->default_width);
            $res &= Configuration::updateValue('ANGARSLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('ANGARSLIDER_PAUSE', $this->default_pause);
            $res &= Configuration::updateValue('ANGARSLIDER_LOOP', $this->default_loop);
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            ...
            foreach ($languages as $language) {
                ...
                $slide->url[$language['id_lang']] = 'index.php';
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('ANGARSLIDER_WIDTH');
            $res &= Configuration::deleteByName('ANGARSLIDER_SPEED');
            $res &= Configuration::deleteByName('ANGARSLIDER_PAUSE');
            $res &= Configuration::deleteByName('ANGARSLIDER_LOOP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'angarslider` (
				`id_angarslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_angarslider_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'angarslider_slides` (
			  `id_angarslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_angarslider_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'angarslider_slides_lang` (
			  `id_angarslider_slides` int(10) unsigned NOT NULL,
			  ...
			  PRIMARY KEY (`id_angarslider_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new AngarSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'angarslider`, `'._DB_PREFIX_.'angarslider_slides`, `'._DB_PREFIX_.'angarslider_slides_lang`;
		');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = AngarSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('ANGARSLIDER_SPEED')) || !Validate::isInt(Tools::getValue('ANGARSLIDER_PAUSE')) ||
                !Validate::isInt(Tools::getValue('ANGARSLIDER_WIDTH'))
            ) {
            ...
            foreach ($languages as $language) {
                ...
                if (isset($_FILES['image_'.$language['id_lang']]) &&
                    ...
                    if ($error = ImageManager::validateUpload($_FILES['image_'.$language['id_lang']])) {
                        $errors[] = $error;
                    }elseif (!move_uploaded_file($_FILES['image_'.$language['id_lang']]['tmp_name'], dirname(__FILE__).'/views/img/images/'.$salt.'_'.$_FILES['image_'.$language['id_lang']]['name'])) {
    ...
    public function hookdisplayHeader($params)
    {
        if(Context::getContext()->isMobile())
        {
            ...
            $this->context->controller->addCSS($this->_path.'views/css/angarslider.css');
            $this->context->controller->addJS($this->_path.'views/js/angarslider.js');
            $this->context->controller->addJqueryPlugin(array('bxslider'));
    
            $config = $this->getConfigFieldsValues();
            $slider = array(
                'width' => $config['ANGARSLIDER_WIDTH'],
                'speed' => $config['ANGARSLIDER_SPEED'],
                'pause' => $config['ANGARSLIDER_PAUSE'],
                'loop' => (bool)$config['ANGARSLIDER_LOOP'],
            );
    
            $this->smarty->assign('angarslider', $slider);
    ...
    public function hookdisplayTopColumn($params)
    {
        if(Context::getContext()->isMobile())
        {
            ...
            return $this->display(__FILE__, 'angarslider.tpl', $this->getCacheId());
        }else{
            return '';
        }
    ...
    {
        $this->_clearCache('angarslider.tpl');
    }
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			WHERE hss.`id_angarslider_slides` = hs.`id_angarslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id);
    ...
    public function getSlides($active = null)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_angarslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
			hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
			FROM '._DB_PREFIX_.'angarslider hs
			LEFT JOIN '._DB_PREFIX_.'angarslider_slides hss ON (hs.id_angarslider_slides = hss.id_angarslider_slides)
			LEFT JOIN '._DB_PREFIX_.'angarslider_slides_lang hssl ON (hss.id_angarslider_slides = hssl.id_angarslider_slides)
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'angarslider_slides hss ON (hs.id_angarslider_slides = hss.id_angarslider_slides)
			LEFT JOIN '._DB_PREFIX_.'angarslider_slides_lang hssl ON (hss.id_angarslider_slides = hssl.id_angarslider_slides)
			WHERE hs.`id_angarslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' '));
    ...
    {
        $req = 'SELECT hs.`id_angarslider_slides` as id_slide
				FROM `'._DB_PREFIX_.'angarslider` hs
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = AngarSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'name' => 'ANGARSLIDER_WIDTH',
                        'suffix' => 'pixels'
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->l('Speed'),
                        'name' => 'ANGARSLIDER_SPEED',
                        'suffix' => 'milliseconds',
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'ANGARSLIDER_WIDTH' => Tools::getValue('ANGARSLIDER_WIDTH', Configuration::get('ANGARSLIDER_WIDTH', null, $id_shop_group, $id_shop)),
            'ANGARSLIDER_SPEED' => Tools::getValue('ANGARSLIDER_SPEED', Configuration::get('ANGARSLIDER_SPEED', null, $id_shop_group, $id_shop)),
            'ANGARSLIDER_PAUSE' => Tools::getValue('ANGARSLIDER_PAUSE', Configuration::get('ANGARSLIDER_PAUSE', null, $id_shop_group, $id_shop)),
            'ANGARSLIDER_LOOP' => Tools::getValue('ANGARSLIDER_LOOP', Configuration::get('ANGARSLIDER_LOOP', null, $id_shop_group, $id_shop)),
        );
    }
